...
// ===== Reels galerija: varna, lahka in gladka na mobitelih =====
(function () {
  function initReels() {
    ...
    // ---- Nastavitve (optimizirane za mobilne) ----
    const BASE_SPEED_ATTR  = parseFloat(viewport.dataset.speed || '0.32'); // px/frame
    const HOVER_SPEED_ATTR = parseFloat(viewport.dataset.hoverSpeed || '0.75');
    const MULTIPLIER       = 1.8;  // širina traku ≥ 1.8x viewport (manj klonov → gladkeje)
    const MAX_CLONES       = 40;   // trda kapica proti runaway DOMu

    // mobilni prilagodimo hitrosti (manj dela na frame)
    const isMobile = window.innerWidth < 768;
    let speed      = isMobile ? BASE_SPEED_ATTR * 0.55 : BASE_SPEED_ATTR;
    const hoverSpeed = isMobile ? HOVER_SPEED_ATTR * 0.7 : HOVER_SPEED_ATTR;
    ...
    // Normaliziraj x v intervalu (-rowW, 0] → nikoli prazen levi rob
    ...
    // Hover pospešek (desktop)
    viewport.addEventListener('mouseenter', () => { if (!isMobile) speed = hoverSpeed; });
    viewport.addEventListener('mouseleave', () => { speed = isMobile ? BASE_SPEED_ATTR * 0.55 : BASE_SPEED_ATTR; });
    ...
    // Zaženi samo, ko je viden; če IO ne zazna, fallback start
    let io = null;
    function startOrObserve() {
      if ('IntersectionObserver' in window) {
        ...
        io.observe(viewport);
        // fallback, če IO ne sproži
        setTimeout(() => { if (!running) play(); }, 800);
      } else {
        play();
      }
    }

    // Upoštevaj vidnost zavihka (sleep v ozadju)
    ...
    // Počakaj na slike (vsaj enkrat), nato napolni in zaženi
    ...
    // Spoštuj "reduced motion", razen če ga izrecno ignoriraš z data-ignore-reduced-motion="true"
    const ignoreRM = viewport.dataset.ignoreReducedMotion === 'true';

    (async function start() {
      if (!ignoreRM && window.matchMedia?.('(prefers-reduced-motion: reduce)').matches) {
        speed = 0; pause();
      }
